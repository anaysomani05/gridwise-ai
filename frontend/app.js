// GridWise AI — Dashboard logic (mock-first, live-when-ready)
// Backend contract: POST {backendUrl}/optimize  -> see MOCK_OPTIMIZE
// Agent contract:   POST {agentUrl}/explain     -> see MOCK_EXPLAIN
...
const MOCK_OPTIMIZE = {
  ...
  baseline: { start: "2026-04-25T18:00:00Z", end: "2026-04-25T22:00:00Z", emissions_kg: 14.8 },
  optimized: { start: "2026-04-26T01:00:00Z", end: "2026-04-26T05:00:00Z", emissions_kg: 10.9 },
  metrics: { co2_saved_kg: 3.9, percent_reduction: 26.35, deadline_met: true },
  timeseries: [
    { timestamp: "2026-04-25T18:00:00Z", signal: 410 },
    { timestamp: "2026-04-25T19:00:00Z", signal: 430 },
    { timestamp: "2026-04-25T20:00:00Z", signal: 420 },
    { timestamp: "2026-04-25T21:00:00Z", signal: 400 },
    { timestamp: "2026-04-25T22:00:00Z", signal: 360 },
    { timestamp: "2026-04-25T23:00:00Z", signal: 320 },
    { timestamp: "2026-04-26T00:00:00Z", signal: 300 },
    { timestamp: "2026-04-26T01:00:00Z", signal: 260 },
    { timestamp: "2026-04-26T02:00:00Z", signal: 250 },
    { timestamp: "2026-04-26T03:00:00Z", signal: 270 },
    { timestamp: "2026-04-26T04:00:00Z", signal: 280 },
    { timestamp: "2026-04-26T05:00:00Z", signal: 310 },
  ],
  reasoning: {
...
const MOCK_EXPLAIN = {
  text_explanation:
    "Running immediately would overlap higher‑carbon evening hours. GridWise shifts the job into a cleaner overnight window while still meeting the deadline, cutting emissions by ~26% (about 3.9 kg CO₂ avoided).",
};

let CHART = null;
let CURRENT_OPTIMIZE = MOCK_OPTIMIZE;
...
function buildPayload() {
  return {
    ...
    duration_hours: Number($("durationHours").value) || 1,
...
async function callExplain(opt) {
  ...
  try {
    const url = `${s.agentUrl.replace(/\/$/, "")}/explain`;
    const body = {
      region: (opt.request && opt.request.region) || $("region").value,
      job: (opt.request) || buildPayload(),
...
function onLoadDemo() {
  $("region").value = MOCK_OPTIMIZE.request.region;
  $("durationHours").value = MOCK_OPTIMIZE.request.duration_hours;
  $("powerKw").value = MOCK_OPTIMIZE.request.power_kw;
  $("startAfter").value = "2026-04-25T18:00:00Z";
  $("deadline").value = MOCK_OPTIMIZE.request.deadline;
  renderOptimize(MOCK_OPTIMIZE);
  renderExplain(MOCK_EXPLAIN);
  setText("reqStatus", "Loaded demo scenario");
}
...
function bind() {
  ...
  $("loadDemoInlineBtn").addEventListener("click", onLoadDemo);
